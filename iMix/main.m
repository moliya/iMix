...
//  iMix
//
//  Created by carefree on 2019/3/12.
...
void generateImportString(NSString *directory);
...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        if (outDirString) {
            ...
            //生成import字符串
            generateImportString(outDirString);
            printf("生成垃圾代码完成\n");
        }
    }
    return 0;
...
void generateImportString(NSString *directory) {
    ...
    for (NSString *fileName in files) {
        ...
        [fileContent appendFormat:@"#import \"%@\"\n", fileName];
    }
    [fileContent writeToFile:[directory stringByAppendingPathComponent:@"importHeaders.h"] atomically:YES encoding:NSUTF8StringEncoding error:nil];
}
